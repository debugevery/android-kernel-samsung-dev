...
#define VCLK  //zjz为像素时钟信号（用于锁存图像数据的像素时钟）
#define VDEN //zjz为数据有效标志信号
#define VD //zjz为图像的数据信号。
#define HSYNC			0x0220//作为行同步信号的 HSYNC，每发出一个脉冲都表明新的一行图像资料开始发送。
#define VSYNC			0x0224//作为帧同步信号的 VSYNC，每发出一个脉冲，都意味着新的一屏图像数据开始发送
