...
//snd_pcm_ops 中的所有操作都需事先通过snd_pcm_substream_chip()获得xxxchip指针
#define snd_pcm_substream_chip(substream) ((substream)->private_data)////从子流获得 xxxchip 指针
...
struct snd_pcm_hardware{//硬件描述（snd_pcm_hardware 结构体）包含了基本硬件配置的定义，
unsigned int info; 	/* SNDRV_PCM_INFO_*   标识 PCM 设备的类型和能力，形式为 SNDRV_PCM_ INFO_XXX。
info 字段至少需要定义是否支持 mmap，当支持时，应设置 SNDRV_PCM_INFO_MMAP 标志；当硬件支持 interleaved 或 non-interleaved 格式时，应 设置 SNDRV_PCM_INFO _INTERLEAVED 或 SNDRV_PCM_INFO_NONINTERLEAVED 标志；如果都支持，则两者都可设置。*/
u64   formats; 	/* SNDRV_PCM_FMTBIT_*  PCM 设备支持的格式，形式为 SNDRV_PCM_FMTBIT_XXX，如果
设备支持多种模式，应将各种模式标志进行"或"操作。*/
unsigned int rates; 	/* SNDRV_PCM_RATE_*   PCM 设备支持的采样率，形式如 SNDRV_PCM_RATE_XXX，如果
支持连续的采样率，则传递 CONTINUOUS。 */
...
size_t period_bytes_min; 	/* 最小周期大小  :PCM 中断产生的周期*/ 
...
//snd_pcm_ops  中的 所 有 操作都 需 事 先 通过  snd_pcm_substream_chip() 获 得xxxchip 指针
struct snd_pcm_ops {
	int (*open)(struct snd_pcm_substream *substream);//当一个 PCM 子流被打开时，open()函数将被调用
	int (*close)(struct snd_pcm_substream *substream);//当 PCM 子流被关闭时，close()函数将被调用
	int (*ioctl)(struct snd_pcm_substream * substream,
		     unsigned int cmd, void *arg);/*驱动中通常可以给ioctl()成员函数传递通用的snd_pcm_lib_ioctl()函数*/
	int (*hw_params)(struct snd_pcm_substream *substream,//硬件参数:将在应用程序设置硬件参数（PCM 子流的周期大小、缓冲区大小和格式等）的时候被调用
	...
	int (*copy)(struct snd_pcm_substream *substream, int channel,//缓冲区复制
...
//snd_pcm_runtime 中的大多数记录对被声卡驱动操作集中的函数是只读的，仅
//仅  PCM  中间层可从更新或修改这些信息，但是硬件描述、中断回调函数、DMA
//缓冲区信息和私有数据是例外的。
struct snd_pcm_runtime {//PCM 信息运行时 结构体
	/* -- Status -- */
	/* 状态 */
	...
	struct snd_pcm_hardware hw;//预先定义的硬件描述
...
struct snd_pcm_substream {
	...
	/* -- runtime information -- */
	struct snd_pcm_runtime *runtime;//PCM 运行时信息指针
        /* -- timer section -- */
