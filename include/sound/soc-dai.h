 ...
 *
 * Digital Audio Interface (DAI) API.数字音频接口API
 */
 ...
 * interface.  
 */  
struct snd_soc_dai_driver { //in kernel 3.0------------------------  
    /* DAI description */  
 ...
 */  
struct snd_soc_dai {  //in kernel 3.0------------------------ 
 ...
 * Holds runtime data for a DAI.
 */
struct snd_soc_dai {//CODEC DAI（Digital Audio Interfaces）和配置PCM,形容playback、capture的属性以及DAI接口的操作
	...
	/* ops */
	struct snd_soc_dai_ops *ops;//描述CODEC的时钟、PLL以及各式设置，主要包括set_sysclk()、set_pll()、set_clkdiv()、set_fmt()等成员函数。
	...
	struct snd_soc_codec *codec;//对CODEC本身I/O控制以及动态音频电源管理（Dynamic Audio Power Management，DAPM）的描述。它描述I2C、SPI或AC'97如何读写CODEC寄存器并容纳DAPM链表，核心成员为read()、write()、hw_write()、hw_read()、dapm_widgets、dapm_paths等。
	unsigned int active;
