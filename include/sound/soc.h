...
/*
 * Convenience kcontrol builders一组宏来描述Codec 的mixer 控制
 *//*参数xname是mixer的名字，reg是控制该mixer的寄存器，shift对应寄存器内的位，max是进行操作时的最大数，invert表明是否极性倒序或翻转*/
...
/* SoC audio ops 音频流开始采集和播放时的一些动作*/
struct snd_soc_ops {
...
/* codec driver */  
struct snd_soc_codec_driver {  //in kernel 3.0------------------------ 
    /* driver ops */  
...
struct snd_soc_codec { //in kernel 3.0------------------------ 
...
struct snd_soc_dai_link  {
	char *name;			/* Codec name Machine驱动将会利用这些名字去匹配已经在系统中注册的platform，codec，dai*/
	...
	int (*init)(struct snd_soc_codec *codec);/*codec/machine 特定的初始化*/
