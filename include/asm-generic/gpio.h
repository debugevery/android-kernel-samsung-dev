...
/*gpio实现为一组可用的 gpio_chip, 由驱动传入对应 gpio的全局序号 去 request, dataout ,datain, 
free. 这时会调用gpio_chip中具体的实现。寄存器读写函数:   __raw_writel()   __raw_writeb()   __raw_readl()   __raw_readb()*/

//struct gpio_chip:表示一个gpio controller.通过这个结构抽象化所有的 GPIO源，而让板上其它的模块可以用相同的接口调用使用这些GPIO。
struct gpio_chip {
	const char		*label;
	struct device		*dev;
	struct module		*owner;
