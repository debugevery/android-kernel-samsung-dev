...
struct inode {
	...
	struct list_head	i_list;	// 索引节点链表	/* backing dev IO list */
	struct list_head	i_sb_list;//
	struct list_head	i_dentry;//目录项链表
	unsigned long		i_ino;//节点号
	atomic_t		i_count;//引用记数
	unsigned int		i_nlink;//访问权限控制
	uid_t			i_uid;//使用者id
	gid_t			i_gid;//使用者id组
	dev_t			i_rdev;//实设备标识符
	...
	struct timespec		i_mtime;// 最后访问时间 
	...
	const struct file_operations	*i_fop;	/* 默认的索引节点操作former ->i_op->default_file_ops */
