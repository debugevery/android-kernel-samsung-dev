/*
 *  Digital Audio (PCM) abstract layer========================PCM抽象接口层
...
参数capture_count 表示该pcm将会有几个capture substream。
 ...
 第 1 个参数是 card 指针，第 2 个是标识字符串，第 3 个是 PCM 设备索引（0 表
示第 1 个 PCM 设备），第 4 和第 5 个分别为播放和录音设备的子流数。当存在多个子 流时，需要恰当地处理 open()、
close()和其他函数。在每个回调函数中，可以通过 snd_pcm_substream 的 number 成员得知目前操作的究竟是哪个子流
*/
