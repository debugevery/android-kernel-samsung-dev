...
注册一个混音器，第一个参数 fops 即是文件操作接口，第二个参数 dev 是设备编号，如果填入-1，则系统自动分配一个设备编号。mixer 是一个典型的字 符设备，因此编码的主要工作是实现 file_operations 中的 open()、ioctl()等函数。
mixer 接口 file_operations 中的最重要函数是 ioctl()，它实现混音器的不同 I/O 控 制命令，
 */
 ...
 *	and will always allocate them as a matching pair - eg dsp3/audio3
 用于注册一个 dsp 设备，第一个参数 fops 即是文件操作接口，第二个参数 dev 是设备编号，如果填入?1，则系统自动分 配一 个 设 备编 号 。 dsp  也是 一个典型 的字 符 设备，因 此 编 码的 主要 工作 是 实现 file_operations 中的 read()、write()、ioctl()等函数。
dsp 接口 file_operations 中的 read()和 write()函数非常重要，read()函数从音频控制 器中获取录音数据到缓冲区并复制到用户空间，write()函数从用户空间复制音频数据 到内核空间缓冲区并最终发送到音频控制器。
dsp 接口 file_operations 中的 ioctl()函数处理对采样率、量化精度、DMA 缓冲区块
大小等参数设置 I/O 控制命令的处理。 在数据从缓冲区复制到音频控制器的过程中，通常会使用 DMA，DMA 对声卡而言非常重要。*/
