...
//利用card->instantiated来判断该卡是否已经实例化，如果已经实例化则直接返回，否则遍历每一对dai_link，
//进行codec、platform、dai的绑定工作
...
/* probes a new socdev 在板驱动中注册"soc-audio"设备会引起两者的匹配，触发soc_probe的调用，它正是整个ASoC驱动初始化的入口。。

它先从platform_device参数中取出snd_soc_card，然后调用snd_soc_register_card，通过snd_soc_register_card，为snd_soc_pcm_runtime数组
申请内存，每一个dai_link对应snd_soc_pcm_runtime数组的一个单元，然后把snd_soc_card中的dai_link配置复制到相应的snd_soc_pcm_runtime中，
最后，大部分的工作都在snd_soc_instantiate_card中实现*/
...
/* ASoC platform driver 在板驱动中注册"soc-audio"设备会引起两者的匹配，触发soc_probe的调用，它正是整个ASoC驱动初始化的入口。。*/
