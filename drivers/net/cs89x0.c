...
/* First, a few definitions that the brave might change.
   ...
   slots. */
/* The cs8900 has 4 IRQ pins, software selectable. cs8900_irq_map maps==========映射系统中断号
   them to system IRQ numbers. This mapping is card specific and is set to
   the configuration of the Cirrus Eval board for this chip. */
#if defined(CONFIG_SH_HICOSH4)
//指定CS8900A使用的资源,根据原理图,确认网卡的地址和中断号
/*一.   根据原理图,确认网卡的地址和中断号
地址:0x19000000
中断号: EINT9
...
*/
static unsigned int netcard_portlist[] /*__used __initdata */ =   { 0x0300, 0};
static unsigned int cs8900_irq_map[] = {1,0,0,0};//指定CS8900A使用的资源,根据原理图,确认网卡的地址和中断号
...
struct net_local {//CS8900  网卡设备定义的私有信息结构体
	struct net_device_stats stats;/* 网络设备状态结构体 */
	int chip_type;		/* 区分芯片类型one of: CS8900, CS8920, CS8920M */
	char chip_revision;	/* 芯片版本字母revision letter of the chip ('A'...) */
	int send_cmd;		/* 发送命令the proper send command: TX_NOW, TX_AFTER_381, or TX_AFTER_ALL */
...
/* Check for a network adaptor of this type, and return '0' iff one exists.
   ...
   Return 0 on success.
   */
//CS8900网卡驱动如果以模块方式挂接，函数init_module就是入口；   
//如果是直接编译到kernel里，那么函数cs89x0_probe才是入口。在此入口函数中，将完成网卡驱动的各项初始化
//。如注册虚拟地址，设备号，中断号，以及各个相关寄存器的初始化。cs89x0_probe函数里会去调用真正的初始化函数cs89x0_probe1。 
...
/*zjz add 设置 S3C2410A ARM 处理器的片选，第 12 行设置 ARM 与CS8900 网卡对应的中断
	...
	if (io > 0x1ff)	{	/* Check a single specified location. */
		...
		err = -ENXIO;
	} else {//循环检索 netcard_portlist[ ]数组中定义的基地址处 是否存在 CS8900 网卡。
		for (port = netcard_portlist; *port; port++) {
			//cs89x0_probe1()函数读取 vCS8900_BASE  + 0x300 基地址处的 CS8900 相关的寄存器，以"反证法"验证网卡的存在，并获取 CS8900所使用的硬件资源。
...
static const struct net_device_ops net_ops = {
	.ndo_open		= net_open,//打开网络设备,输入ifconfig命令时，最后就会调用到这个函数======= 被__dev_open()调用，在/net/core/dev.c中
	.ndo_stop		= net_close,//关闭网络设备======= 被__dev_close()调用，在/net/core/dev.c中
	.ndo_tx_timeout		= net_timeout,//发送超时的处理函数
	.ndo_start_xmit 	= net_send_packet,//通知发送网络数据======= 被dev_hard_start_xmit()调用，在/net/core/dev.c中
	.ndo_get_stats		= net_get_stats,//获取状态======= 被dev_get_stats()调用，在/net/core/dev.c中
	.ndo_set_multicast_list = set_multicast_list,//设置接收模式，如是全部接收（Promiscuous模式），接收广播等======= 被__dev_set_rx_mode()调用，在/net/core/dev.c中
	.ndo_set_mac_address 	= set_mac_address,//======= 被ndo_set_mac_address()调用，在/net/core/dev.c中
#ifdef CONFIG_NET_POLL_CONTROLLER
	.ndo_poll_controller	= net_poll_controller,//======= 被netpoll_poll_dev()调用，在/net/core/netpoll.c中
#endif
	.ndo_change_mtu		= eth_change_mtu,//======= 被dev_set_mtu()调用，在/net/core/dev.c中
	.ndo_validate_addr	= eth_validate_addr,//======= 被__dev_open()调用，在/net/core/dev.c中
};
...
/* This is the real probe routine.  Linux has a history of friendly device
   ...
   Return 0 on success.
 */
//网络初始化被调用的路径为：init->do_basic_setup->do_initcalls->net_olddevs_init->ethif_probe2->probe_list2->cs89x0_probe->cs89x0_probe1
//CS8900网卡驱动如果以模块方式挂接，函数init_module就是入口；   
//如果是直接编译到kernel里，那么函数cs89x0_probe才是入口。在此入口函数中，将完成网卡驱动的各项初始化。如注册虚拟地址，设备号，中断号，以及各个相关寄存器的初始化。 
static int __init  cs89x0_probe1(struct net_device *dev, int ioaddr, int modular)
{
	...
	dev->base_addr = ioaddr;//很重要的成员变量赋值：
	...
	/*通过request_region函数注册虚拟地址 WTF is going on here? */
  ...
  //从后面的代码中可以看到，还需要确定cs8900的ID号后才能真正确保cs8900存在。   
	...
	writeword(ioaddr, ADD_PORT, PP_ChipID);//这里表示扫描到cs8900，按照数据手册写
	...
	/* get the chip type */
	rev_type = readreg(dev, PRODUCT_ID_ADD);//读芯片类型
	lp->chip_type = rev_type &~ REVISON_BITS;
	lp->chip_revision = ((rev_type & REVISON_BITS) >> 8) + 'A';

	/* Check the chip type and revision in order to set the correct send command
	CS8920 revision C and CS8900 revision F can use the faster send. */
	//默认每次传输381字节，根据数据手册可以传输的字节,选项有5、381、1021、all四个，但这里的驱动不支持1021字节的选项
	lp->send_cmd = TX_AFTER_381;/* 根据芯片类型和版本确定正确的发送命令 */
	...
	if (1) {
		...
		while (--cnt > 0) {
			...
			switch (j & 0x0fff) {
			case PP_IA:
				for (i = 0; i < ETH_ALEN/2; i++) {//网卡的MAC地址,一般都是从eeproom中读出来再存放到该变量中，当然也可以根据自己的需要手动赋值。 
...
#endif
		//从EEPROM中读出配置信息，并填充dev结构的相关域。{//读取SelfST寄存器，并判断EEPROM
        //是否存在若存在，则判断是否读取操作成功。上述条件满足,则读出EEPROM中的配置信息填充dev相关域。
        if ((readreg(dev, PP_SelfST) & (EEPROM_OK | EEPROM_PRESENT)) ==
	      (EEPROM_OK|EEPROM_PRESENT)) {
	        /* Load the MAC. */
		for (i=0; i < ETH_ALEN/2; i++) {
	                unsigned int Addr;
			Addr = readreg(dev, PP_IA+i*2);//读网卡的MAC地址,一般都是从eeproom中读出来再存放到该变量中，当然也可以根据自己的需要手动赋值。 
		...
		lp->adapter_cnf = 0;
		i = readreg(dev, PP_LineCTL);//读取LineCTL寄存器，确定MAC配置和物理接口
	...
	/*.//以下检查EEPROM的相关信息======First check to see if an EEPROM is attached. */
	...
	if (lp->chip_type != CS8900 &&
		...
		if (!dev->irq)
			dev->irq = i;//很重要的成员变量赋值
	} else {
		i = lp->isa_config & INT_NO_MASK;//由于没有EEPROM，所以lp->isa_config=0
		if (lp->chip_type == CS8900) {
#ifdef CONFIG_CS89x0_NONISA_IRQ
		        i = cs8900_irq_map[0];//指定CS8900A使用的资源,根据原理图,确认网卡的地址和中断号
#else
			/* Translate the IRQ using the IRQ mapping table. */
			if (i >= ARRAY_SIZE(cs8900_irq_map))//求cs8900_irq_map数据元个数
				printk("\ncs89x0: invalid ISA interrupt number %d\n", i);
			else
				i = cs8900_irq_map[i];//指定CS8900A使用的资源,根据原理图,确认网卡的地址和中断号
...
//net_open函数是一个专门用来注册网络设备中断号的函数，输入ifconfig命令时，最后就会调用到这个函数。
static int net_open(struct net_device *dev)
{//需要了解CS8900的寄存器来设置
	struct net_local *lp = netdev_priv(dev);
	int result = 0;
...
#endif
/* And 2.3.47 had this: */
		writereg(dev, PP_BusCTL, ENABLE_IRQ | MEMORY_ON);//在这个函数中要把中断号设置一下。 

		for (i = 2; i < CS8920_NO_INTS; i++) {
			if ((1 << i) & lp->irq_map) {//数据接收到将产生一个中断
...
/* And 2.3.47 had this: */
		...
		ret = request_irq(dev->irq, net_interrupt, 0, dev->name, dev);//数据接收到将产生一个中断
		if (ret) {
...
// net_send_parcket为内核提供了数据包发送功能，该函数在cs89x0_probe1中被赋予了net_device的hard_start_xmit域，当内核需要发送数据包时，将调用dev-> hard_start_xmit完成最后的数据包发送。
static netdev_tx_t net_send_packet(struct sk_buff *skb,struct net_device *dev)
{
	...
	spin_lock_irqsave(&lp->lock, flags);//获得自旋锁，以便进入临界区
	netif_stop_queue(dev); //通知内核暂停内核与驱动程序间的数据传递，也即告诉内核不要向skb缓冲区填充数据。

	/*/* 初始化硬件发送序列 */ //初始化cs8900的发送对列，主要为写命令和数据长度，为数据发送做准备initiate a transmit sequence */
	writeword(dev->base_addr, TX_CMD_PORT, lp->send_cmd);
	writeword(dev->base_addr, TX_LEN_PORT, skb->len);

	/*/* 检测硬件是否处于发送 READY 状态 */ //查看cs8900是否为发送分配了地址空间。Test to see if the chip has allocated memory for the packet */
	...
	/*//将数据交给cs8900发送 Write the contents of the packet */
...
static irqreturn_t net_interrupt(int irq, void *dev_id)
{//数据接收到将产生一个中断，需要了解CS8900芯片的中断处理流程
	...
	while ((status = readword(dev->base_addr, ISQ_PORT))) {
		...
		handled = 1;
		switch(status & ISQ_EVENT_MASK) {/* 读取中断事件类型 */    
		case ISQ_RECEIVER_EVENT:
			/*获得数据包 Got a packet(s). */
			net_rx(dev);//net_rx函数的主要功能是获得数据包,从cs8900的片上数据缓冲区中将数据传送给sk_buff缓冲区
			break;
...
//net_rx函数的主要功能是从cs8900的片上数据缓冲区中将数据传送给sk_buff缓冲区
static void net_rx(struct net_device *dev)
{
	...
	length = readword(ioaddr, RX_FRAME_PORT);//读取接收数据包的长度,获取cs8900片上缓冲区的长度
	...
	/*分配新的套接字缓冲区和数据缓冲区 Malloc up new buffer. */
	...
	//该函数增加skb的data和tail，
              ...
              //节。所以这里的主要作用是字对齐。	/* longword align L3 header */
	skb_reserve(skb, 2);
		// 读取硬件上接收到的数据，skb_put函数的作用是更新skb的tail和len成员，也即在缓冲区尾部添加数据，该函数返
	...
	if (length & 1) //因为前面length以字对齐，如果length为单字节，所以这里应该补上最后一个字节
...
{//alloc_etherdev是alloc_netdev的封装函数，它负责在内核空间为 net_device结构体分配内存(kmalloc)。
	struct net_device *dev = alloc_etherdev(sizeof(struct net_local));
	struct net_local *lp;
	int ret = 0;
	...
	dev->irq = irq;//中断号，如果没有自定义，且有默认值
	dev->base_addr = io;//基地址，如果没有自定义，且有默认值
	lp = netdev_priv(dev);
