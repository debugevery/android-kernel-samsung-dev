/*给出了一个 PCI 接口网络设备驱动程序的“骨架”，其pci_driver  中 probe()成员函数 
netdrv_init_one()及其调用的 netdrv_init_board()完成了PCI 设备初始化及对应的网络设备
注册工作，
				       ...
				       void **ioaddr_out)
{
	...
	*dev_out = NULL;
	...
	dev = alloc_etherdev(sizeof(*tp));/* 分配 ethernet 设备 */
	if (dev == NULL) {
	...
	rc = pci_enable_device(pdev);/* 使能设备 */
	if (rc)
		goto err_out;
	/* 获取 I/O 和内存基地址 */
	...
	/* 确保 PCI 基地址寄存器 0 是 PIO *//* make sure PCI base addr 0 is PIO */
	...
	/* 确保 PCI 基地址寄存器 1 是 MMIO *//* make sure PCI base addr 1 is MMIO */
	...
	/* 检查 weird/broken PCI 区域报告 *//* check for weird/broken PCI region reporting */
...
#else
	/* ioremap MMIO 区域 *//* ioremap MMIO region */
	...
	/* 检查芯片已完成复位，让芯片进入低能耗模式 *//* Bring the chip out of low-power mode. */
	/* <在此插入设备特定的代码> *//* <insert device-specific code here> */
...
/* when built into the kernel, we only print version if device is found */
	...
	pci_set_drvdata(pdev, dev);//设置 PCI 私有数据
...
static void __devexit netdrv_remove_one(struct pci_dev *pdev)
{
	...
	pci_release_regions(pdev);//释放 pci 区域

	free_netdev(dev);//释放 ethernet
