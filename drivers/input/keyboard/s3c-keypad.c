 ...
 * 见Kconfig 此驱动支持depends on CPU_S5PC100 || CPU_S5PC110 || CPU_S5P6442 || CPU_S5PV210
...
static int __init s3c_keypad_probe(struct platform_device *pdev)
{
	...
	//======================初始化s3c_keypad为赋值keypad_timer.data
	...
	s3c_keypad->dev = input_dev;//=============通过此值设置输入设备
	...
	/* Scan timer init */
	init_timer(&keypad_timer);//定义定时器
	keypad_timer.function = keypad_timer_handler;//定时器处理函数
	keypad_timer.data = (unsigned long)s3c_keypad;//定时器处理函数的参数数据=========将s3c_keypad所有信息赋值给keypad_timer.data 
	...
	ret = input_register_device(input_dev);//===================通过input_dev注册输入设备
	if (ret) {
