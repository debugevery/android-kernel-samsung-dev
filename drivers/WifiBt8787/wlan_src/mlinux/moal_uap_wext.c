                ...
                struct iw_point *dwrq, char *extra)
{
    ...
    custom_ie *cptr = NULL;
    int copy_size = 0, travel_len = 0, resp_len = 0;
    ...
    resp_len = misc->param.cust_ie.len;
    while (resp_len > sizeof(t_u16)) {
        cptr =
            (custom_ie *) (((t_u8 *) misc->param.cust_ie.ie_data_list) +
                           travel_len);
        if (cptr->ie_length && (copy_size + cptr->ie_length) <= dwrq->length) {
            memcpy(extra, cptr->ie_buffer, cptr->ie_length);
            copy_size += cptr->ie_length;
        }
