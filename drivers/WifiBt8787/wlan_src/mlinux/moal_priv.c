...
woal_warm_reset(moal_private * priv)
{
    ...
    if (bss_info.is_hs_configured) {
        if (MLAN_STATUS_SUCCESS != woal_cancel_hs(priv, MOAL_IOCTL_WAIT)) {
            ret = -EFAULT;
            goto done;
        }
    }
 ...
 */
static int
woal_wl_fw_reload(moal_private * priv, struct iwreq *wrq)
{
    int ret = 0;
    t_u8 buf[256];
...
#define MIN_FW_RELOAD_CMD_LEN   8       /* strlen("FW_PATH=") */
    if ((wrq->u.data.length - 1) <= MIN_FW_RELOAD_CMD_LEN)
        return -EINVAL;

    memset(buf, 0, sizeof(buf));
    memcpy(buf, wrq->u.data.pointer, wrq->u.data.length);
    if (strncmp(buf, "FW_PATH=", MIN_FW_RELOAD_CMD_LEN)) {
        PRINTM(MERROR, "String 'FW_PATH=' not found in the buffer\n");
        ret = -EINVAL;
        goto done;
    }

    PRINTM(MINFO, "New FW reloaded path=%s\n", buf + MIN_FW_RELOAD_CMD_LEN);
...
woal_do_ioctl(struct net_device *dev, struct ifreq *req, int cmd)
{
    ...
    case WOAL_FROYO_WL_FW_RELOAD:
        ret = woal_wl_fw_reload(priv, wrq);
        break;
