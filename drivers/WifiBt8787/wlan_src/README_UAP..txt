...
2) FOR DRIVER INSTALL
	...
	a) Copy sd8787_uap.bin | ... to /lib/firmware/mrvl/ directory, create the directory 
	   if it doesn't exist.
	b) Install uAP driver,
		insmod mlan.ko
		insmod sd8688.ko | sd8686.ko | ...
...
3) FOR DRIVER PROC & DEBUG
	...
	cmd_pending=<0/1, no pending command/there is a pending command>
	num_cmd_h2c_fail = <number of commands failed to send to device>
        num_tx_h2c_fail = <number of data packets failed to send to device>
	drvdbg= <driver debug message control>
...
sys_cfg_11n [ENABLE] [HTCAP] [AMPDU]
...
---------------
    ...
    MODE: band config mode.
          Bit 0:  auomatic channel selection (ACS) enable/disable
...
---------------
	...
	ISDEFAULT:   0: KeyIndex is not the default
        ...
        INDEX - Only INDEX will get the key setting for the perticular
                KeyIndex.
...
--------------
    ...
    The supported options are:
        ...
        If only PROTOCOL is provided, pairwise cipher for that protocol is dislayed.
...
----------------
    ...
    The supported options are:
        ...
        SLEEP_PARAM:  
			...
			The value of MAX_SLEEP should be <= 32000 us(when ctrl: 0).
        ...
        INACT_PARAM: 
			...
			The value of MIN_AWAKE should be >= 10 us.
	...
	./uaputl.exe powermode 1
		Enable periodic DTIM power save mode.

   	./uaputl.exe powermode 2
		Enable inactivity based power save mode.   

   	./uaputl.exe powermode 1 1 1 20000 20000 
		Enable periodic DTIM power save mode, and set sleep param, enable protection, 
		and set minimum sleep during to 20000 us, maximum sleep duration to 20000 us

   	./uaputl.exe powermode 2 2 400000 10000 10000
		Enable inactivity based power save mode and set inactivity sleep param with 
		inactivity timeout 400000 us, minimum awake duration to 10000us , maximum awake duration to 10000 us

	./uaputl.exe powermode 1 2 400000 10000 10000
		Enable periodic DTIM power save mode, and set inactivity sleep param 
		with inactivity timeout 400000 us, minimum awake duration to 10000us ,
	...
	./uaputl.exe powermode 
     ...
     Note: Before changing power save mode from 1 to 2 or vice-versa, power save 
	 	 	should be disabled using "powermode 0" command.
...
sys_cfg_wmm
----------------
    "./uaputl.exe sys_cfg_wmm [qosinfo=<qos_info>]
                              [AC_BE AIFSN ECW_MAX ECW_MIN TX_OP]
                              [AC_BK AIFSN ECW_MAX ECW_MIN TX_OP]
                              [AC_VI AIFSN ECW_MAX ECW_MIN TX_OP]
                              [AC_VO AIFSN ECW_MAX ECW_MIN TX_OP]" 
    ...
    The supported option are:
        qos_info: qos information. User can set only MSB. Valid values are 0x80 and 0x00.
        AC_BE: 0
        AC_BK: 1
        AC_VI: 2
        AC_V0: 3
...
-----------
	    ...
	    Note:- This command should only be issue in BSS_STOP state.
