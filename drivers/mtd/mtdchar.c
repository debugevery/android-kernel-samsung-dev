...
mtdchar.c 实现了字符设备接口，通过它，用户可以直接操作 Flash 设备。通过
read()、write()系统调用可以读写 Flash，通过一系列 IOCTL 命令可以获取 Flash 设备
信息、擦除Flash、读写NAND 的OOB、获取OOB layout及检查 NAND 坏块等。 
 */
...
static int mtd_open(struct inode *inode, struct file *file)
{
	...
	mfi->ino = mtd_ino;
	mfi->mtd = mtd;
	file->private_data = mfi;//获取文件私有数据mtd_info,来自平台驱动初始化
...
static ssize_t mtd_read(struct file *file, char __user *buf, size_t count,loff_t *ppos)
{
	...
	while (count) {
		...
		default:
			ret = mtd->read(mtd, *ppos, len, &retlen, kbuf);//调用nand_read
		}
