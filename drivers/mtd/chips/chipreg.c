...
第一个参数为探测的接口类型,do_map_probe()会根据传入的参数 name 通过 get_mtd_chip_driver()得到具体的
MTD 驱动，调用与接口对应的probe()函数探测设备.
利用map_info中的配置，do_map_probe()可以自动识别支持 CFI或 JEDEC（电子
元件工业联合会）接口的 Flash 芯片，MTD 以后会自动采用适当的命令参数对 Flash
进行读写或擦除。 */
