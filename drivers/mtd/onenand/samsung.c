 ...
 * Kconfig:支持 MTD_ONENAND && (ARCH_S3C64XX || ARCH_S5PC100 || ARCH_S5PV210)
 *  Copyright 漏 2008-2010 Samsung Electronics
...
/* S5PC110 specific definitions ==========特殊定义*/
...
{//DMA操作
	...
	writel(src, base + S5PC110_DMA_SRC_ADDR);//写DMA起始地址
	writel(dst, base + S5PC110_DMA_DST_ADDR);//写DMA结尾地址

	if (direction == S5PC110_DMA_DIR_READ) {
		writel(S5PC110_DMA_SRC_CFG_READ, base + S5PC110_DMA_SRC_CFG);//写DMA起始配置寄存器
		writel(S5PC110_DMA_DST_CFG_READ, base + S5PC110_DMA_DST_CFG);//写DMA结尾配置寄存器
	} else {
		writel(S5PC110_DMA_SRC_CFG_WRITE, base + S5PC110_DMA_SRC_CFG);
		writel(S5PC110_DMA_DST_CFG_WRITE, base + S5PC110_DMA_DST_CFG);
	}

	writel(count, base + S5PC110_DMA_TRANS_SIZE);//配置传输SIZE
	...
	do {
		status = readl(base + S5PC110_DMA_TRANS_STATUS);//传输状态
		if (status & S5PC110_DMA_TRANS_STATUS_TE) {
			writel(S5PC110_DMA_TRANS_CMD_TEC,
					base + S5PC110_DMA_TRANS_CMD);
			return -EIO;
		}//循环判断DMA传输状态标志位，判断是否结束
...
{//onenand中断子函数
	...
	status = readl(base + S5PC110_INTC_DMA_STATUS);//读取中断状态寄存器
...
static int s3c_onenand_probe(struct platform_device *pdev)
{
	...
	onenand->pdev = pdev;
	onenand->type = platform_get_device_id(pdev)->driver_data;//区分6410，110等

	s3c_onenand_setup(mtd);//===================onenand相关设置
	...
	if (onenand->type != TYPE_S5PC110) {//如果CPU不是TYPE_S5PC110
	...
	{ /* S5PC110=================DMA支持 */
		...
		s5pc110_dma_ops = s5pc110_dma_poll;//如果不使用DMA中断的默认操作函数
		/* Interrupt support ==================DMA支持中断*/
		r = platform_get_resource(pdev, IORESOURCE_IRQ, 0);
		if (r) {//如果使用DMA中断则使用下列操作函数
	...
	if (onenand_scan(mtd, 1)) {//==============================探测onenand设备
		err = -EFAULT;
		goto scan_failed;
	}
...
#ifdef CONFIG_MTD_PARTITIONS   //========================MTD分区
