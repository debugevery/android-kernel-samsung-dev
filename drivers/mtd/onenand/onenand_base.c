/*
 *  linux/drivers/mtd/onenand/onenand_base.c=============核心函数
 *
 *  Copyright 漏 2005-2009 Samsung Electronics
 *  Copyright 漏 2007 Nokia Corporation
 *
...
static int onenand_probe(struct mtd_info *mtd)
{
	...
	ret = this->chip_probe(mtd);//使用samsung.c中定义的s3c_onenand_probe（）
	if (ret)
		return ret;

	/* Read manufacturer and device IDs from Register =======读取机器和设备ID*/
	maf_id = this->read_word(this->base + ONENAND_REG_MANUFACTURER_ID);
	dev_id = this->read_word(this->base + ONENAND_REG_DEVICE_ID);
	ver_id = this->read_word(this->base + ONENAND_REG_VERSION_ID);
	this->technology = this->read_word(this->base + ONENAND_REG_TECHNOLOGY);

	/* Flash device information =======打印设备信息*/
	onenand_print_device_info(dev_id, ver_id);
	this->device_id = dev_id;
	this->version_id = ver_id;

	/* Check OneNAND features =========检查和设置onenand特性*/
	onenand_check_features(mtd);

	density = onenand_get_density(dev_id);//从设备ID中获取密度参数？
	if (FLEXONENAND(this)) {
		this->dies = ONENAND_IS_DDP(this) ? 2 : 1;
		/* Maximum possible erase regions=========得到最大擦除范围 */
		mtd->numeraseregions = this->dies << 1;
	 ...
	 * mtd->size represents the actual device size.===========//设备实际大小
	 */
	this->chipsize = (16 << density) << 20;

	/* OneNAND page size & block size */
	/* The data buffer size is equal to page size========页尺寸 */
...
int onenand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	if (!this->read_word)//========在samsung.c中已经自定义了，否则使用这里的默认函数
	...
	if (onenand_probe(mtd))//检测onenand设备
		return -ENXIO;
	...
	/* Allocate buffers, if necessary ==============分配buffer*/
	if (!this->page_buf) {
	...
	/* Fill in remaining MTD driver data=============/dev/设备文件操作函数 */
...
#ifdef CONFIG_MTD_ONENAND_OTP//===========OTP操作
