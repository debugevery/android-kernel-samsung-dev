 ...
 * 支持depends on MTD_NAND && (ARCH_S5PC1XX || ARCH_S5PC11X || ARCH_S5PV2XX || ARCH_S5PV210)
...
/*
 * This function is called before encoding ecc codes to ready ecc engine.使能硬件ECC
 */
...
/*
 * This function is called immediately after encoding ecc codes.编码ECC
...
/*
 * This function determines whether read data is good or not.纠错数据
...
static int s3c_nand_probe(struct platform_device *pdev, enum s3c_cpu_type cpu_type)
{
	...
	for (i = 0; i < plat_info->chip_nr; i++) {//===================有的地方把此处封装成一个函数
...
#if defined(CONFIG_MTD_NAND_S3C_HWECC)//---------------------硬件ECC操作
