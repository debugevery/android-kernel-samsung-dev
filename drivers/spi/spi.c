 ...
 * with other readonly (flashable) information about mainboard devices.
 ...
 //结构体boardinfo管理多个结构体spi_board_info*/
struct boardinfo {
...
从spi_alloc_device()函数返回后继续初始化spi_device结构体的chip_select，mode，modalias, controller_data等变量，这里的proxy->modalias被初始化为chip->modalias， */
...
int spi_add_device(struct spi_device *spi)
{
	...
	//设置spi模式和时钟速率,会调用spi->master->setup函数设置
...
{
	struct spi_device	*proxy;//这个结构很重要，以后就是通过这个结构来操作实际的spi设备
	int			status;
	...
	/* 初始化spi_device 结构各成员 *///各个成员赋值，都是用我们注册的板级信息
 ...
 * any embedded pointers (platform_data, etc), theyre copied as-is.
 函数很简单，利用定义的spi_board_info信息，填充了boardinfo结构，并挂到board_list链表。*/
int __init
spi_register_board_info(struct spi_board_info const *info, unsigned n)
{
	struct boardinfo	*bi;
	//为结构体boardinfo 分配内存空间
...
int spi_setup(struct spi_device *spi)
{
	...
	status = spi->master->setup(spi);//调用s3c64xx_spi_setup（）
 ...
 * which are wrappers around this core asynchronous primitive.)
 异步的SPI传输函数，可以用于在中端或者某些不能睡眠的上下文场合，当然也可以用于睡眠的场合。*/
int spi_async(struct spi_device *spi, struct spi_message *message)
{
	...
	return master->transfer(spi, message););//调用s3c64xx_spi_transfer传输数据
}
		...
		u8 *rxbuf, unsigned n_rx)
{
	...
	memset(x, 0, sizeof x);
	/* 留意到没有：tx和rx个占一个工作添加到message的struct spi_transfer链表
	里，稍后被bitbang_work从链表里提出来处理(后面会讲到) */   
	if (n_tx) {
		x[0].len = n_tx;
	...
	/* do the i/o *///同步io，等待spi传输完成，然后返回用户所接收的数据和状态
