 ...
 *	"s3cfb";210特定驱动.linux/arch/arm/plat-s5p/devs.c
 * Core file for Samsung Display Controller (FIMD) driver=======================核心文件
 *	三星SOC芯片上集成了显示控制器（有称作fimd的，Fully Interactive Mobile Display）。
 ...
 * published by the Free Software Foundation.
 ...
 Framebuffer驱动的移植步骤
1）初始化framebuffer信息，分配内存，地址映射；
2）参考LCD硬件手册，配置LCD参数信息，填充fb_info结构体，进行设备注册；
3）参考LCD控制器使用手册，实现fp_ops的接口函数；
*/
...
{//初始化fb_fix_screeninfo 和 fb_var_screeninfo以备以后直接拿来用，寄存器里已经在s3cfb_init_global设置了
	...
	win->id = id;
	win->path = DATA_PATH_DMA;//DMA方式
	win->dma_burst = 16;
	alpha->mode = PLANE_BLENDING;

	fb->fbops = &s3cfb_ops;//基本画图函数
...
//?????s3cfb_register_framebuffer??,???????????register_framebuffer??5?window???                            
//???CONFIG_FRAMEBUFFER_CONSOLE ??, ????????kconfig????                                                            
...
//??konfig????/drivers/video/console/kconfig                                                                                   
//                                                                                                                                   
...
static int __devinit s3cfb_probe(struct platform_device *pdev)
{
	...
	fbdev->dev = &pdev->dev;
	//================================电源管理
	fbdev->regulator = regulator_get(&pdev->dev, "pd");//从平台设备结构体中获取电源管理相关信息
	if (!fbdev->regulator) {
	...
	//================================/*获取LCD参数信息*/	
	pdata = to_fb_plat(&pdev->dev);//获取平台数据
	if (!pdata) {
	...
	//================================初始化
	s3cfb_set_vsync_interrupt(fbdev, 1);/*设置VSYNC中断，一帧完成产生中断*/
	s3cfb_set_global_interrupt(fbdev, 1);/*设置全局总中断*/vvvvv
	s3cfb_init_global(fbdev)/*fb设备平台数据参数信息初始化,写入寄存器*/
	...
	s3cfb_set_window(fbdev, pdata->default_win, 1);//设置默认窗口

	s3cfb_display_on(fbdev);//开电源

	fbdev->irq = platform_get_irq(pdev, 0);//申请中断
...
static struct platform_driver s3cfb_driver = {
	...
	.driver = {
		   .name = "s3cfb",//S3CFB_NAME,"s3cfb";210特定驱动.linux/arch/arm/plat-s5p/devs.c
		   .owner = THIS_MODULE,
	},
};
