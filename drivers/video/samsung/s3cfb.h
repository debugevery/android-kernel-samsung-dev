...
/*
 * struct s3cfb_lcd_polarity================================判断LCD的极性，是否反转
 * @rise_vclk:	if 1, video data is fetched at rising edge
 * @inv_hsync:	if HSYNC polarity is inversed=====反转
...
/*
 * struct s3cfb_lcd_timing=====================就是LCD datasheet中的参数值
...
struct s3cfb_lcd {
	...
	int	bpp;
	int	freq;//帧频率
	struct	s3cfb_lcd_timing timing;//典型TFT时序相关参数值
	struct	s3cfb_lcd_polarity polarity;//极性是否反转
...
struct s3cfb_window {
	...
	int			x;
	int			y;
	enum			s3cfb_data_path_t path;//常用DMA还是FIFO
...
struct s3cfb_global {
	...
	void __iomem		*regs;
	struct mutex		lock;
	struct device		*dev;//基本设备结构体
	struct clk		*clock;
	struct regulator	*regulator;
	int			irq;
	struct fb_info		**fb;//fb的相关信息
	...
	struct s3cfb_lcd	*lcd;//LCD芯片相关参数
