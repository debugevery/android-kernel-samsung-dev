 ...
 *
下面看看 s3c2410fb_check_var函数要干些什么事，要说到这个函数，还得提到fb_var_screeninfo结构类型，与它对应的是 fb_fix_screeninfo结构类型。这两个类型分别代表了显示屏的属性信息，这些信息可以分为可变属性信息（如：颜色深度，分辨率等）和不可变 的信息（如帧缓冲的其实地址）。既然fb_var_screeninfo表示了可变的属下信息，那么这些可变信息就应该有一定范围，否则显示就会出问题， 所以s3c2410fb_check_var函数的功能就是要在LCD的帧缓冲驱动开始运行之前将这些值初始到合法的范围内。知道了 s3c2410fb_check_var要做什么，再去阅读s3c2410fb_check_var函数的代码就没什么问题了。 */
...
/* 下面检查fb_var_screeninfo的xres和yres的值是否超出法定范围，如果查出将其设定为正确的值。*/
	...
	var->transp.offset = 0;
	var->transp.length = 0;
	/* set r/g/b positions *//* 羡慕开始检查bpp（表示用多少位表示一个像素），如果不合法，将其设置正确*/
...
/*该函数的主要作用是让处理器的LCD控制器的三个地址寄存器指向正确的位置，这个位置就是LCD的缓冲区，详细的情况可以参见s3c2410的用户手册。*/
...
//设置颜色寄存器。它需要6个参数，分别代表寄存器编号，红色，绿色，蓝色，透明和fb_info结构。
				  ...
				  enum s3c_drv_type drv_type)
{
	...
	mach_info = pdev->dev.platform_data;//获取lcd相关寄存器配置信息
	if (mach_info == NULL) {
		dev_err(&pdev->dev,
