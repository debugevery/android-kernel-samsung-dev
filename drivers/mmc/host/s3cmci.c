...
static inline bool s3cmci_host_usedma(struct s3cmci_host *host)
{
	...
	return false;
#elif defined(CONFIG_MMC_S3C_DMA)//根据内核配置选项判断是否使用DMA
	return true;
#else
	return host->dodma;
#endif
...
{
#ifdef CONFIG_MMC_S3C_PIO//根据内核配置选项判断PIO代码是否有效
	return true;
#else
	return false;
#endif
 ...
 * @host: The host to check.检查中断是否正在响应，判断是否结束
 
 *万一主控制器检测中断失败，需要检查是否产生中断响应，或者中断是否结束。
 * Test to see if the SDIO interrupt is being signalled in case the
 * controller has failed to re-detect a card interrupt. Read GPE8 and
 * see if it is low and if so, signal a SDIO interrupt.通过读GPE8是否为低，就可以判断
 
 *一般称为是否中断结束，当IRQ使能中，表示正在中断中
...
{
	if (host->sdio_irqen) {
		if (gpio_get_value(S3C2410_GPE(8)) == 0) {//获取GPE8的值
...
{//从FIFO 的数据读出
	...
	while ((fifo = fifo_count(host))) {
		if (!host->pio_bytes) {
			res = get_data_buffer(host, &host->pio_bytes,//从FIFO 获取数据
					      &host->pio_ptr);
			if (res) {
		...
		/* If we have reached the end of the block, we can在块结尾可以读1到3个字节
		 * read a word and get 1 to 3 bytes.  If we in the
		 * middle of the block, we have to read full words,在块的中间，必须读4字节
...
{//下面初始化过一个名为pio_tasklet的tasklet，用于实现中断的底半部 机制，底半部服务函数为pio_tasklet，
...
/*
 * ISR for SDI Interface IRQ（================SDI接口中断）
...
/*
 * ISR for the CardDetect Pin（============卡拨插引脚中断）
*/
...
{//sysfs显示的匹配
	...
	host->debug_state = debugfs_create_file("state", 0444,
						...
						&s3cmci_fops_state);//通过状态相关操作函数在sysfs中显示
	...
	host->debug_regs = debugfs_create_file("regs", 0444,
					       host->debug_root, host,
					       &s3cmci_fops_regs);//SDI执行流程在sysfs中的显示的相关操作
...
static int __devinit s3cmci_probe(struct platform_device *pdev)
{
	...
	mmc = mmc_alloc_host(sizeof(struct s3cmci_host), &pdev->dev);//=======调用mmc_rescan函数=====================
	if (!mmc) {
		ret = -ENOMEM;
		goto probe_out;
	}
	...
	if (!host->pdata) {
		pdev->dev.platform_data = &s3cmci_def_pdata;//默认平台数据
