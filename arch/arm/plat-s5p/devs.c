...
static struct resource s3cfb_resource[] = {//LCD控制器的资源信息
	[0] = {
...
//导出定义的LCD平台设备，好在mach-t34h.c的smdkv210_devices[]中添加到平台设备列表中
EXPORT_SYMBOL(s3c_device_fb);
...
//==========LCD所有信息就是一个指针赋值给device->platform_data,不管信息有多
...
void __init s3c_jpeg_set_platdata(struct s3c_platform_jpeg *pd)
{
	...
	//将pd结构体信息复制到npd内存中
	npd = kmemdup(pd, sizeof(struct s3c_platform_jpeg), GFP_KERNEL);//将pd结构体信息复制到npd内存中
	if (!npd)
		printk(KERN_ERR "%s: no memory for platform data\n", __func__);
	else
