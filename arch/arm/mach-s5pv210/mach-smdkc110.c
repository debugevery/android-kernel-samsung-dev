...
/*
¥Æø⁄≥ı ºªØΩ·ππÃÂ
UARTµƒ…Ë÷√£®UCONn°¢ULCONn°¢UFCONnºƒ¥Ê∆˜µ»£©
*/
...
//”…s3c24xx_init_uarts≥ı ºªØ°£ƒ⁄∫À≈‰÷√—°œÓCONFIG_DEBUG_S3C_UART=2£¨‘Ú π”√COM2¿¥µ˜ ‘
static struct s3c2410_uartcfg smdkc110_uartcfgs[] /* __initdata */ = {
	{
		.hwport		= 0,
		.flags		= 0,
		.ucon		= S5PV210_UCON_DEFAULT,//UCON : ±÷”—°‘Ò01 = EXT_UCLK0: DIV_VAL1) = (EXT_UCLK0 / (bps x 16) ) ®C1
         // | Tx÷–∂œ¿‡–Õlevel | Rx÷–∂œ¿‡–Õlevel | ø™Rx≥¨ ±÷–∂œ | ∑¢ÀÕ÷–∂œªÚ¬÷—Øƒ£ Ω | Ω” ’÷–∂œªÚ¬÷—Øƒ£ Ω
         //EXT_UCLK0 clock is external clock.(XpwmECLK PAD input) Œ™∫Œ «EXT_UCLK0?
		.ulcon		= S5PV210_ULCON_DEFAULT,//ULCON:≈‰÷√Œ™8bit
...
static struct s5p_media_device smdkc110_media_devs[] = {
	...
	[4] = {
		.id = S5P_MDEV_FIMC2,
		.name = "fimc2",// Fully Interactive Mobile Camera £®ÕÍ»´Ωªª• Ω“∆∂Ø…„œÒª˙£©
		.bank = 1,
	...
	[6] = {
		.id = S5P_MDEV_FIMD,
		.name = "fimd",// Fully Interactive Mobile Display £®ÕÍ»´Ωªª• Ω“∆∂Øœ‘ æ…Ë±∏£©
		.bank = 1,
...
//=======================================================================================================================
#if defined(CONFIG_TOUCHSCREEN_S3C)////”√”⁄…Ë÷√¥•√˛∆¡–≈œ¢
...
//=============∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›platform_data
static struct s3c_ts_mach_info s3c_ts_platform /* __initdata */ = {
	.delay                  = 10000,//AD◊™ªª—” ±
	.presc                  = 49,//∑÷∆µ
	.oversampling_shift     = 2,// ≤…—˘µƒ ˝æ›
	.resol_bit              = 12,//◊™ªªæ´∂»
	.s3c_adc_con            = ADC_TYPE_2,
};
//Ω´TSΩ¯––¡À∫Ø ˝≤Œ ˝£®…œ√ÊΩ·ππ–≈œ¢£©µƒ“ª–©≥ı ºªØ£¨
//…Ë÷√ƒ¨»œ÷µ£¨µ´ «≤¢√ª”––¥»Î”≤º˛ºƒ¥Ê∆˜£¨ΩˆΩˆ «±£¥Ê‘⁄platform_data÷–
void __init s3c_ts_set_platdata(struct s3c_ts_mach_info *pd)
{
	...
	if (npd) {
		memcpy(npd, pd, sizeof(*npd));//Ω´pdΩ·ππÃÂ–≈œ¢∏¥÷∆µΩnpdƒ⁄¥Ê÷–
		s3c_device_ts.dev.platform_data = npd;//Ω´pd÷∏’Î∏≥÷µ
	} else {
...
#else//±ªdrivers/input/keyboard/s3c-keypad.cµ˜”√===========…Ë÷√œ‡πÿºƒ¥Ê∆˜
...
#endif
//=======================================================================================================================
#ifdef CONFIG_DM9000//…Ë÷√œ‡πÿºƒ¥Ê∆˜
...
#endif
//=======================================================================================================================
#ifdef CONFIG_FB_S3C_LTE480WV//»˝–«4.8¥Á“∫æß∆¡LTE480WV

#ifdef LCD_800480
static struct s3cfb_lcd lte480wv = {//»˝–«4.8¥Á“∫æß∆¡LTE480WV
#if 0
...
#else
	.width	= S5PV210_LCD_WIDTH,//∆¡ƒªøÌ∂»
	.height	= S5PV210_LCD_HEIGHT,//∆¡ƒª∏ﬂ∂»
	//“‘œ¬“ª–©≤Œ ˝‘⁄…œ√Êµƒ ±–ÚÕº∑÷Œˆ÷–Ω≤µΩπ˝,∏˜≤Œ ˝µƒ÷µ«Î∏˙æ›æﬂÃÂµƒLCD∆¡ ˝æ› ÷≤·Ω·∫œ…œ√Ê ±–Ú∑÷Œˆ¿¥…Ë∂®
	.bpp	= 24,//…´Œªƒ£ Ω
	.freq	= 60,//œÒÀÿ ±÷”

	.timing = {// ±–Úœ‡πÿ≤Œ ˝-----------------------------------------
		.h_fp	= 8,//––«–ªª£¨¥”Õ¨≤ΩµΩªÊÕº÷Æº‰µƒ—”≥Ÿ
		.h_bp	= 40,//––«–ªª£¨¥”ªÊÕºµΩÕ¨≤Ω÷Æº‰µƒ—”≥Ÿ
		.h_sw	= 3,//ÀÆ∆ΩÕ¨≤Ωµƒ≥§∂»
		.v_fp	= 5,//÷°«–ªª£¨¥”Õ¨≤ΩµΩªÊÕº÷Æº‰µƒ—”≥Ÿ
		.v_fpe	= 1,//÷°«–ªª£¨¥”ªÊÕºµΩÕ¨≤Ω÷Æº‰µƒ—”≥Ÿ
		.v_bp	= 22,
		.v_bpe	= 1,
		.v_sw	= 1,//¥π÷±Õ¨≤Ωµƒ≥§∂»
	},
...
//»˝–«4.8¥Á“∫æß∆¡LTE480WV============∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›platform_data
...
/*’‚±ﬂ∂®“Â¡À2∏ˆSPIøÿ÷∆∆˜µƒPlatform device£¨id∑÷±Œ™°∞0£¨1°±£¨
name∂ºŒ™°∞spidev°±£¨±‰¡øresource÷–∂®“Â¡À  ≈‰∆˜µƒºƒ¥Ê∆˜ª˘µÿ÷∑°£*/
static struct spi_board_info s3c_spi_devs[] /* __initdata */ = {
	[0] = {
		.modalias        = "spidev",	 /* device node name *///∂‘”¶spidev.c÷–µƒ"spidev",≤¢µ˜”√probe∫Ø ˝Ω¯––∆•≈‰
		.mode            = SPI_MODE_0,	/* ¥À¥¶—°‘ÒæﬂÃÂ ˝æ›¥´ ‰ƒ£ ΩCPOL=0, CPHA=0 */
		.max_speed_hz    = 10000000,//ƒ¨»œ//◊Ó¥Ûµƒspi ±÷”∆µ¬ 
		/* Connected to SPI-0 as 1st Slave */
		.bus_num         = 0,//…Ë±∏¡¨Ω”‘⁄spiøÿ÷∆∆˜0…œ
		.irq             = IRQ_SPI0,
		.chip_select     = 0,//∆¨—°œﬂ∫≈£¨‘⁄S5PC100µƒøÿ÷∆∆˜«˝∂Ø÷–√ª”– π”√À¸◊˜Œ™∆¨—°µƒ“¿æ›£¨∂¯ «—°‘Ò¡Àœ¬Œƒcontroller_data¿Ôµƒ∑Ω∑®
...
#endif
//=======================================================================================================================
#ifdef CONFIG_S5P_ADC//∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›platform_data
...
/* Interface setting ∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›platform_data*/
...
//=======================================================================================================================
#ifdef CONFIG_VIDEO_JPEG_V2//∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›platform_data
...
static struct i2c_board_info i2c_devs2[] /* __initdata */ = {
	{
		...
		.platform_data = &max8998_pdata,//µÁ≥ÿ≤‚¡ø∆ΩÃ® ˝æ›
#else
		/* The address is 0xCC used since SRAD = 0 */
...
//≥ı ºªØµƒ∆ΩÃ®…Ë±∏
/*ƒ⁄∫À∆Ù∂Ø∫Û£¨ ◊œ»ππ‘Ï¡¥±ÌΩ´√Ë ˆ…Ë±∏µƒplatform_deviceππ‘Ï◊È÷Ø∆¿¥£¨
µ√µΩ“ª∏ˆ…Ë±∏µƒ¡–±Ì£ªµ±º”‘ÿƒ≥∏ˆ«˝∂Ø≥Ã–Úµƒplatform_driverΩ·ππ ±£¨
 π”√“ª–©∆•≈‰∫Ø ˝¿¥ºÏ≤È«˝∂Ø≥Ã–Úƒ‹∑Ò÷ß≥÷’‚–©…Ë±∏£¨≥£”√µƒºÏ≤È∑Ω∑®∫‹ºÚµ•£∫±»Ωœ«˝∂Ø≥Ã–Ú∫Õ…Ë±∏µƒ√˚≥∆°£
*/
static struct platform_device *smdkc110_devices[] /* __initdata */ = {
...
#ifdef CONFIG_FB_S3C
	&s3c_device_fb,//LCD µƒFB…Ë±∏:"s3cfb"| drivers/video/samsung/s3cfb.c | drivers/video/fbmem.c
#endif
#ifdef CONFIG_DM9000
...
#endif
#ifdef CONFIG_VIDEO_FIMC//Fully Interactive Mobile Camera £®ÕÍ»´Ωªª• Ω“∆∂Ø…„œÒª˙£©
	&s3c_device_fimc0,
	&s3c_device_fimc1,
	&s3c_device_fimc2,
#endif
	...
	&s5pv210_device_spi1,
#endif
#ifdef CONFIG_S5PV210_POWER_DOMAIN//===========–Ë“™µÁ‘¥π‹¿Ìµ•‘™
	&s5pv210_pd_audio,
	&s5pv210_pd_cam,
	&s5pv210_pd_tv,
	&s5pv210_pd_lcd,//LCDµƒµÁ‘¥…Ë±∏
	&s5pv210_pd_g3d,
	&s5pv210_pd_mfc,
#endif
...
{
	//÷˜“™ «◊ˆÕ‚Œß…Ë±∏µƒŒÔ¿Ìµÿ÷∑µΩ–Èƒ‚µÿ÷∑µƒ”≥…‰
	s5p_init_io(NULL, 0, S5P_VA_CHIPID);//µ˜”√linux/arch/arm/plat-s3c64xx/cpu.c÷–∫Ø ˝
	//∆µ¬ µƒ≥ı ºªØ
	s3c24xx_init_clocks(24000000);	
	s5pv210_gpiolib_init();
	//¥Æø⁄µƒ≥ı ºªØ£∫Àƒ∏ˆ¥Æø⁄∂ºƒ¨»œ…Ë÷√¡À
...
static void __init smdkc110_machine_init(void)
{
...
#ifdef CONFIG_ANDROID_PMEM//—°‘Ò–‘◊¢≤·¥À∆ΩÃ®…Ë±∏
	platform_device_register(&pmem_gpu1_device);
#endif
	pm_power_off = smdkc110_power_off ;

#ifdef CONFIG_ANDROID_PMEM
	android_pmem_set_platdata();//¿‡À∆s3c_ts_set_platdata,…Ë÷√∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›
#endif
	/* i2c */
	//Ω´S3C2440…œµƒI2Cøÿ÷∆∆˜Ω¯––¡À“ª–©≥ı ºªØ£¨
	//…Ë÷√ƒ¨»œ÷µ£¨µ´ «≤¢√ª”––¥»Î”≤º˛ºƒ¥Ê∆˜£
	//ΩˆΩˆ «±£¥Ê‘⁄s3c2410_platform_i2cΩ·ππÃÂ
	s3c_i2c0_set_platdata(NULL);
	s3c_i2c1_set_platdata(NULL);//¿‡À∆s3c_ts_set_platdata,…Ë÷√∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›
	s3c_i2c2_set_platdata(NULL);

	sound_init();
	
	//’‚∏ˆ∫Ø ˝◊¢≤·static struct i2c_board_infoΩ·ππ£¨¥ÀΩ·ππ «¥˙ÃÊI2C…Ë±∏µƒ∞Âº∂◊¢≤·
	...
	smdkc110_dm9000_set();
#endif
//=====================================LCDÀ˘”––≈œ¢æÕ «“ª∏ˆ÷∏’Î∏≥÷µ∏¯device->platform_data,≤ªπ‹–≈œ¢”–∂‡…Ÿ
#ifdef CONFIG_FB_S3C_LTE480WV
	s3cfb_set_platdata(&lte480wv_fb_data);//¿‡À∆s3c_ts_set_platdata,…Ë÷√∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›
#endif
	...
	/*Ω´spi Platform device◊¢≤·µΩPlatform◊‹œﬂ…œ£¨ÕÍ≥…◊¢≤·∫Û£¨ºƒ¥Ê∆˜µƒª˘µÿ÷∑µ»
	–≈œ¢ª·‘⁄…Ë±∏ ˜÷–√Ë ˆ¡À£¨¥À∫Û÷ª–Ë¿˚”√platform_get_resourceµ»±Í◊ºΩ”ø⁄◊‘
	∂ØªÒ»°º¥ø…£¨ µœ÷¡À«˝∂Ø∫Õ◊ ‘¥µƒ∑÷¿Î°£∞—spi_board_info◊¢≤·“™¡¥±Ìboard_list…œ°£*/
	spi_register_board_info(s3c_spi_devs, ARRAY_SIZE(s3c_spi_devs));
#endif

#if defined(CONFIG_TOUCHSCREEN_S3C)
	//Ω´TSΩ¯––¡À∫Ø ˝≤Œ ˝µƒ“ª–©≥ı ºªØ£¨
	//…Ë÷√ƒ¨»œ÷µ£¨µ´ «≤¢√ª”––¥»Î”≤º˛ºƒ¥Ê∆˜£¨ΩˆΩˆ «±£¥Ê‘⁄platform_data÷–
	s3c_ts_set_platdata(&s3c_ts_platform);//∫Ø ˝∫Õ≤Œ ˝‘⁄“ª∆£¨»›“◊¿ÌΩ‚
#endif

#if defined(CONFIG_S5P_ADC)
	s3c_adc_set_platdata(&s3c_adc_platform);//¿‡À∆s3c_ts_set_platdata,…Ë÷√∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›
#endif
	...
	/* fimc :Fully Interactive Mobile Camera £®FIMCÕÍ»´Ωªª• Ω“∆∂Ø…„œÒª˙£©*/
	s3c_fimc0_set_platdata(&fimc_plat_lsi);//¿‡À∆s3c_ts_set_platdata,…Ë÷√∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›
...
#ifdef CONFIG_VIDEO_FIMC_MIPI
	s3c_csis_set_platdata(NULL);//¿‡À∆s3c_ts_set_platdata,…Ë÷√∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›
#endif

#ifdef CONFIG_VIDEO_JPEG_V2
	s3c_jpeg_set_platdata(&jpeg_plat);//¿‡À∆s3c_ts_set_platdata,…Ë÷√∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›
#endif

#ifdef CONFIG_VIDEO_MFC50
	/* mfc */
	s3c_mfc_set_platdata(NULL);//¿‡À∆s3c_ts_set_platdata,…Ë÷√∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›
#endif
...
#ifdef CONFIG_S3C_DEV_HSMMC
	s5pv210_default_sdhci0();//¿‡À∆s3c_ts_set_platdata,…Ë÷√∑Ω Ω≤ªÕ¨,Õ¨—˘…Ë÷√∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›
#endif
#ifdef CONFIG_S3C_DEV_HSMMC1
	s5pv210_default_sdhci1();//¿‡À∆s3c_ts_set_platdata,…Ë÷√∑Ω Ω≤ªÕ¨,Õ¨—˘…Ë÷√∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›
#endif
#ifdef CONFIG_S3C_DEV_HSMMC2
	s5pv210_default_sdhci2();//¿‡À∆s3c_ts_set_platdata,…Ë÷√∑Ω Ω≤ªÕ¨,Õ¨—˘…Ë÷√∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›
#endif
#ifdef CONFIG_S3C_DEV_HSMMC3
	s5pv210_default_sdhci3();//¿‡À∆s3c_ts_set_platdata,…Ë÷√∑Ω Ω≤ªÕ¨,Õ¨—˘…Ë÷√∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›
#endif
#ifdef CONFIG_S5PV210_SETUP_SDHCI
	s3c_sdhci_set_platdata();//¿‡À∆s3c_ts_set_platdata,…Ë÷√∆ΩÃ®…Ë±∏œ‡πÿ ˝æ›
#endif

#ifdef CONFIG_BACKLIGHT_PWM
	smdk_backlight_register();//…Ë÷√smdk_backlight_device∆ΩÃ®…Ë±∏
#endif
...
MACHINE_START(SMDKV210, "SMDKV210")
	...
	.phys_io	= S3C_PA_UART & 0xfff00000,//bak¥Æø⁄µƒ∆ ºµÿ÷∑°£“™ø¥ƒ„µƒ∞Â◊”£¨ «≤ª «ŒÔ¿ÌioæÕ «’‚∏ˆ∆ ºµÿ÷∑
	.io_pg_offst	= (((u32)S3C_VA_UART) >> 18) & 0xfffc,//io“≥±Ìµƒ∆´“∆
	.boot_params	= S5P_PA_SDRAM + 0x100,//∆Ù∂Ø≤Œ ˝“≥±Ìµƒ∆´“∆
	//÷ÿ“™µƒ∞Âº∂≥ı ºªØ∫Ø ˝
	.init_irq	= s5pv210_init_irq,//≥ı ºªØIRQ÷–∂œ
	.map_io		= smdkc110_map_io,//map_io≥…‘±º¥ƒ⁄∫ÀÃ·π©∏¯”√ªßµƒ¥¥Ω®Õ‚…ËI/O◊ ‘¥µΩƒ⁄∫À–Èƒ‚µÿ÷∑æ≤Ã¨”≥…‰±ÌµƒΩ”ø⁄∫Ø ˝
	.init_machine	= smdkc110_machine_init,//bak≥ı ºªØ∆ΩÃ®
	.timer		= &s5p_systimer,//∂® ±∆˜
MACHINE_END
/*
∞¥∂®“Â’πø™∫Û£∫
static const struct machine_desc __mach_desc_SMDK6410 \
 ...
 __attribute__((__section__(".arch.info.init"))) = {
	.nr = MACH_TYPE_SMDK6410,//ø™∑¢∞Â‘⁄linux÷–µƒª˙∆˜∫≈°£
	.name = "SMDK6410",//ø™∑¢∞Â√˚≥∆
	.phys_io = S3C_PA_UART & 0xfff00000,//∆ ºIOŒÔ¿Ìµÿ÷∑
	.io_pg_offst = (((u32)S3C_VA_UART) >> 18) & 0xfffc,
	.boot_params = S3C64XX_PA_SDRAM + 0x100,//ƒ⁄∫À∆Ù∂Ø≤Œ ˝µƒµÿ÷∑
	.init_irq = s5pv210_init_irq,//÷–∂œ≥ı ºªØ∫Ø ˝
	.map_io = smdkc110_map_io,//IO”≥…‰∫Ø ˝£®‘⁄’‚¿Ô–ﬁ∏ƒ ±÷”∆µ¬ £©
