...
struct s3c_adc_client {//ADC客户端：定义有用的信息
...
static inline void s3c_adc_convert(struct adc_device *adc)//ADC转换
{
...
static inline void s3c_adc_select(struct adc_device *adc,//ADC选择
...
int s3c_adc_start(struct s3c_adc_client *client,//ADC转换开始
...
int s3c_adc_read(struct s3c_adc_client *client, unsigned int ch)//ADC读取
{
...
struct s3c_adc_client *s3c_adc_register(struct platform_device *pdev,//ADC寄存器
...
static irqreturn_t s3c_adc_irq(int irq, void *pw)//ADC中断
{
...
static int s3c_adc_probe(struct platform_device *pdev)//ADC检测
{
...
static struct platform_driver s3c_adc_driver = {//ADC平台设备============================================
