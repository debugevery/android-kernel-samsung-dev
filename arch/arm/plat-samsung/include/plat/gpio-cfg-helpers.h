 ...
 * by disabling interrupts.
 ...
 //这里的set_config是一个函数指针，由后面的分析知道，如果针对GPA,该函数指针指向
 s3c_gpio_setcfg_s3c24xx_a , 如果针对GPX应该是指向s3c_gpio_setcfg_s3c24xx——但
 发现，如果是其他GPX，根本没有定义set_config!!! （这个问题已经解决，见后
 文s3c24xx_gpiolib_init函数，事实上，其余的config的确指向s3c_gpio_do_setcfg函数）
*/
