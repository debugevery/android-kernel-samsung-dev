...
/* Serial port registrationsI/O设备 */
...
static struct resource s3c_wdt_resource[] = {//Watchdog资源结构体
	[0] = {
		.start = S3C24XX_PA_WATCHDOG,//Watchdog所在I/O口开始地址
		.end   = S3C24XX_PA_WATCHDOG + S3C24XX_SZ_WATCHDOG - 1,
		.flags = IORESOURCE_MEM,//I/O内存资源
	},
	[1] = {
		.start = IRQ_WDT,//中断开始向量号
		.end   = IRQ_WDT,
...
struct platform_device s3c_device_wdt = {//Watchdog平台设备结构体
	.name		  = "s3c2410-wdt",//设备名
	.id		  = -1,
	.num_resources	  = ARRAY_SIZE(s3c_wdt_resource),
	.resource	  = s3c_wdt_resource,//资源
};
