...
/* GPIO bank summary:
 *
 * Bank	GPIOs引脚	Style引脚位数	SlpCon	ExtInt Group
 * A	8	4Bit	Yes	1=========================8*4bit=32bit
 * B	7	4Bit	Yes	1
 * C	8	4Bit	Yes	2
 * D	5	4Bit	Yes	3
 * E	5	4Bit	Yes	None
 * F	16	2Bit	Yes	4 [1]=====================16*2bit=32bit
 * G	7	4Bit	Yes	5
 * H	10	4Bit[2]	Yes	6===========================
 * I	16	2Bit	Yes	None
 * J	12	2Bit	Yes	None
...
static struct s3c_gpio_chip gpio_4bit[] = {//描述了芯片中所有的GPIO端口（每个引脚为4位时）
	{
		.base	= S3C64XX_GPA_BASE,//// datasheet上地址为===================GPA
		.config	= &gpio_4bit_cfg_eint0111,//  // 设置GPIO的函数指针
		.chip	= {
			.base	= S3C64XX_GPA(0),//基地址，也是偏移量
			.ngpio	= S3C64XX_GPIO_A_NR,
			.label	= "GPA",
		},
	}, {
		.base	= S3C64XX_GPB_BASE,//===================GPB
...
static struct s3c_gpio_chip gpio_2bit[] = {//描述了芯片中所有的GPIO端口（每个引脚为2位时）
	{
...
core_initcall(s3c64xx_gpiolib_init);//===========================最终调用，调用上面所有函数变量
