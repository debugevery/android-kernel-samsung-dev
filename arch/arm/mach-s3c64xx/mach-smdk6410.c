...
#define UFCON S3C2410_UFCON_RXTRIG8 | S3C2410_UFCON_FIFOMODE
 /*
´®¿Ú³õÊ¼»¯½á¹¹Ìå
UARTµÄÉèÖÃ£¨UCONn¡¢ULCONn¡¢UFCONn¼Ä´æÆ÷µÈ£©
*/
...
static struct s3c2410_uartcfg smdk6410_uartcfgs[] /*__initdata */ = {
	[0] = {
		.hwport	     = 0,
		.flags	     = 0,
		.ucon	     = UCON, //Ê±ÖÓÑ¡Ôñ01 = EXT_UCLK0: DIV_VAL1) = (EXT_UCLK0 / (bps x 16) ) ¨C1
         // | TxÖÐ¶ÏÀàÐÍlevel | RxÖÐ¶ÏÀàÐÍlevel | ¿ªRx³¬Ê±ÖÐ¶Ï | ·¢ËÍÖÐ¶Ï»òÂÖÑ¯Ä£Ê½ | ½ÓÊÕÖÐ¶Ï»òÂÖÑ¯Ä£Ê½
         //EXT_UCLK0 clock is external clock.(XpwmECLK PAD input) ÎªºÎÊÇEXT_UCLK0?
		.ulcon	     = ULCON,//ÅäÖÃÎª8bit
...
static struct platform_device smdk6410_lcd_powerdev = {//Æ½Ì¨Éè±¸
	.name			= "platform-lcd",
...
//===========================smdk6410_lcd_pdata====================================================
static struct s3c_fb_pd_win smdk6410_fb_win0 = {//ÏÔÊ¾²ÎÊýÉèÖÃ
...
static struct s3c_fb_platdata smdk6410_lcd_pdata /*__initdata */ = {
	.setup_gpio	= s3c64xx_fb_gpio_setup_24bpp,//ÉèÖÃ24»Ò¶ÈLCDµÄGPIOÅäÖÃ¼Ä´æÆ÷ÎªLCD¹¦ÄÜ
...
static struct platform_device smdk6410_smsc911x = {//Æ½Ì¨Éè±¸
	.name          = "smsc911x",
	.id            = -1,
...
static struct platform_device smdk6410_b_pwr_5v = {//Æ½Ì¨Éè±¸
...
#endif
//===============================================================================
//ÎÒÔÚÕâÌí¼ÓµÄdm9000µØÖ·Ó³Éä
static struct map_desc smdk6410_iodesc[] = {};

//===============================================================================
//³õÊ¼»¯µÄÆ½Ì¨Éè±¸
/*ÄÚºËÆô¶¯ºó£¬Ê×ÏÈ¹¹ÔìÁ´±í½«ÃèÊöÉè±¸µÄplatform_device¹¹Ôì×éÖ¯ÆðÀ´£¬
µÃµ½Ò»¸öÉè±¸µÄÁÐ±í£»µ±¼ÓÔØÄ³¸öÇý¶¯³ÌÐòµÄplatform_driver½á¹¹Ê±£¬
Ê¹ÓÃÒ»Ð©Æ¥Åäº¯ÊýÀ´¼ì²éÇý¶¯³ÌÐòÄÜ·ñÖ§³ÖÕâÐ©Éè±¸£¬³£ÓÃµÄ¼ì²é·½·¨ºÜ¼òµ¥£º±È½ÏÇý¶¯³ÌÐòºÍÉè±¸µÄÃû³Æ¡£
*/
static struct platform_device *smdk6410_devices[] /*__initdata */ = {//°üº¬µÄÆ½Ì¨Éè±¸Êý×é
...
/* VDDARM, BUCK1 on J5 *///Õâ¸ödata±»ÖÃÓÚ ¶ÔÓ¦µÄ platform_device µÄdata×Ö¶ÎÖÐ¡£
...
static struct {
	int regulator;////bakÎÈÑ¹Æ÷Ãû³Æ
	struct regulator_init_data *initdata;//bak³õÊ¼»¯Êý¾Ý
} wm1190_regulators[] = {
...
static int __init smdk6410_wm8350_init(struct wm8350 *wm8350)
{
	...
	/* Instantiate the regulators ³õÊ¼»¯µçÔ´ÎÈÑ¹Æ÷*/
...
#endif
//===============================================================================
static struct i2c_board_info i2c_devs0[] /*__initdata */ = {//ÃèÊöÁË°åÔØµÄËùÓÐI2CÉè±¸ÐÅÏ¢
...
{
	u32 tmp;
	//Ö÷ÒªÊÇ×öÍâÎ§Éè±¸µÄÎïÀíµØÖ·µ½ÐéÄâµØÖ·µÄÓ³Éä
	s3c64xx_init_io(smdk6410_iodesc, ARRAY_SIZE(smdk6410_iodesc));//µ÷ÓÃlinux/arch/arm/plat-s3c64xx/cpu.cÖÐº¯Êý
	//ÆµÂÊµÄ³õÊ¼»¯
	s3c24xx_init_clocks(12000000);//³õÊ¼»¯clockÎª12Mhz
	//´®¿ÚµÄ³õÊ¼»¯£ºËÄ¸ö´®¿Ú¶¼Ä¬ÈÏÉèÖÃÁË
	s3c24xx_init_uarts(smdk6410_uartcfgs, ARRAY_SIZE(smdk6410_uartcfgs));

	/* set the LCD type */
	//Í¨¹ýSPCONÌØÊâ¶Ë¿Ú¹¦ÄÜ¼Ä´æÆ÷ÉèÖÃLCDÀàÐÍ
	tmp = __raw_readl(S3C64XX_SPCON);//¶ÁÈ¡GPIO¼Ä´æÆ÷µÄÖµ
	tmp &= ~S3C64XX_SPCON_LCD_SEL_MASK;
	tmp |= S3C64XX_SPCON_LCD_SEL_RGB;//ÉèÖÃGGB I/FÀàÐÍ
	__raw_writel(tmp, S3C64XX_SPCON);//Ð´ÈëÖµµ½¼Ä´æÆ÷

	/* remove the lcd bypassÍ¨¹ýMIFPCON¼Ä´æÆ÷ÉèÖÃLCDÅÔÂ·Ñ¡Ôñ¿ØÖÆÄ£Ê½ */
	tmp = __raw_readl(S3C64XX_MODEM_MIFPCON);//¶ÁÈ¡GPIO¼Ä´æÆ÷µÄÖµ
	tmp &= ~MIFPCON_LCD_BYPASS;
	__raw_writel(tmp, S3C64XX_MODEM_MIFPCON);//Ð´ÈëÖµµ½¼Ä´æÆ÷
}
...
static void __init smdk6410_machine_init(void)
{
	...
	s3c_i2c0_set_platdata(NULL);//½«S3C2440ÉÏµÄI2C¿ØÖÆÆ÷½øÐÐÁËÒ»Ð©³õÊ¼»¯£¬//ÉèÖÃÄ¬ÈÏÖµ£¬µ«ÊÇ²¢Ã»ÓÐÐ´ÈëÓ²¼þ¼Ä´æÆ÷£
	s3c_i2c1_set_platdata(NULL);//½«S3C2440ÉÏµÄI2C¿ØÖÆÆ÷½øÐÐÁËÒ»Ð©³õÊ¼»¯£¬//ÉèÖÃÄ¬ÈÏÖµ£¬µ«ÊÇ²¢Ã»ÓÐÐ´ÈëÓ²¼þ¼Ä´æÆ÷£
	//ÉèÖÃLCDÆ½Ì¨Êý¾Ý²ÎÊý
	...
	/*Í¨¹ý__raw_readl,__raw_writelº¯ÊýÖ±½ÓÉèÖÃ¼Ä´æÆ÷ configure nCS1 width to 16 bits */
	cs1 = __raw_readl(S3C64XX_SROM_BW) &          //¶ÁÈ¡GPIO¼Ä´æÆ÷µÄÖµ
	...
	/* Í¨¹ý__raw_readl,__raw_writelº¯ÊýÖ±½ÓÉèÖÃ¼Ä´æÆ÷set timing for nCS1 suitable for ethernet chip */
...
/* ÉêÇëÒ»¸ögpio£¬ÆäÊµ¾ÍÊÇ¼ì²é¸ÃgpioÊÇ·ñ¿ÕÏÐ£¬Èç¹û¿ÕÏÐ¾Í¿ÉÒÔgpioÔòÎªÄãÒªÉêÇëµÄÄÄÒ»¸ö¹Ü½Å£¬labelÔòÊÇÎªÆäÈ¡Ò»¸öÃû×Ö¡£*/
	gpio_request(S3C64XX_GPN(5), "LCD power");
	gpio_request(S3C64XX_GPF(13), "LCD power");
	gpio_request(S3C64XX_GPF(15), "LCD power");
	//Õâ¸öº¯Êý×¢²ástatic struct i2c_board_info½á¹¹£¬´Ë½á¹¹ÊÇ´úÌæI2CÉè±¸µÄ°å¼¶×¢²á
	i2c_register_board_info(0, i2c_devs0, ARRAY_SIZE(i2c_devs0));//½«Ò»¸öi2c_board_infoµÄ½á¹¹Ìå×¢²á½øÏµÍ³
	i2c_register_board_info(1, i2c_devs1, ARRAY_SIZE(i2c_devs1));
	//×¢²ásmdk6410ÌØÓÐµÄÉè±¸
	platform_add_devices(smdk6410_devices, ARRAY_SIZE(smdk6410_devices));//bakÌí¼Ó¶à¸öÆ½Ì¨Éè±¸
}

MACHINE_START(SMDK6410, "SMDK6410")
	/* Maintainer: Ben Dooks <ben@fluff.org> */
	.phys_io	= S3C_PA_UART & 0xfff00000,//bak´®¿ÚµÄÆðÊ¼µØÖ·¡£Òª¿´ÄãµÄ°å×Ó£¬ÊÇ²»ÊÇÎïÀíio¾ÍÊÇÕâ¸öÆðÊ¼µØÖ·
	.io_pg_offst	= (((u32)S3C_VA_UART) >> 18) & 0xfffc,//ioÒ³±íµÄÆ«ÒÆ
	.boot_params	= S3C64XX_PA_SDRAM + 0x100,//Æô¶¯²ÎÊýÒ³±íµÄÆ«ÒÆ
//ÖØÒªµÄ°å¼¶³õÊ¼»¯º¯Êý
	.init_irq	= s3c6410_init_irq,//³õÊ¼»¯IRQÖÐ¶Ï
	.map_io		= smdk6410_map_io,//map_io³ÉÔ±¼´ÄÚºËÌá¹©¸øÓÃ»§µÄ´´½¨ÍâÉèI/O×ÊÔ´µ½ÄÚºËÐéÄâµØÖ·¾²Ì¬Ó³Éä±íµÄ½Ó¿Úº¯Êý
	.init_machine	= smdk6410_machine_init,/////bak³õÊ¼»¯Æ½Ì¨
	.timer		= &s3c24xx_timer,//¶¨Ê±Æ÷
MACHINE_END
/*
°´¶¨ÒåÕ¹¿ªºó£º
static const struct machine_desc __mach_desc_SMDK6410 \
 ...
 __attribute__((__section__(".arch.info.init"))) = {
	.nr = MACH_TYPE_SMDK6410,//¿ª·¢°åÔÚlinuxÖÐµÄ»úÆ÷ºÅ¡£
	.name = "SMDK6410",//¿ª·¢°åÃû³Æ
	.phys_io = S3C_PA_UART & 0xfff00000,//ÆðÊ¼IOÎïÀíµØÖ·
	.io_pg_offst = (((u32)S3C_VA_UART) >> 18) & 0xfffc,
	.boot_params = S3C64XX_PA_SDRAM + 0x100,//ÄÚºËÆô¶¯²ÎÊýµÄµØÖ·
	.init_irq = s3c6410_init_irq,//ÖÐ¶Ï³õÊ¼»¯º¯Êý
	.map_io = smdk6410_map_io,//IOÓ³Éäº¯Êý£¨ÔÚÕâÀïÐÞ¸ÄÊ±ÖÓÆµÂÊ£©
