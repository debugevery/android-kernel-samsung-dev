                        ...
                        snd_pcm_access_t access)
{
        ...
        err = snd_pcm_hw_params_any(handle, params);//初始化 hw_params
        if (err < 0) {
        ...
        err = snd_pcm_hw_params_set_access(handle, params, access);//初始化访问权限
        if (err < 0) {
                printf("Access type not available for playback: %s\n", snd_strerror(err));
                return err;
        }
        /* set the sample format */
        err = snd_pcm_hw_params_set_format(handle, params, format);//初始化采样格式
        if (err < 0) {
                printf("Sample format not available for playback: %s\n", snd_strerror(err));
                return err;
        }
        /* set the count of channels */
        err = snd_pcm_hw_params_set_channels(handle, params, channels);//设置通道数量
        if (err < 0) {
        ...
        /* set the stream rate */
        rrate = rate;
        err = snd_pcm_hw_params_set_rate_near(handle, params, &rrate, 0);//设置采样率， 如果硬件不支持我们设置的采样率， 将使用最接近的
        if (err < 0) {
        ...
        err = snd_pcm_hw_params(handle, params);//设置 hw_params
        if (err < 0) {
...
static int xrun_recovery(snd_pcm_t *handle, int err)
{
        if (err == -EPIPE) {    /* under-run */
                err = snd_pcm_prepare(handle);//完成硬件参数设置， 使设备准备好
                if (err < 0)
                      ...
                      snd_pcm_channel_area_t *areas)
{
        ...
        while (1) {
                ...
                cptr = period_size;
                while (cptr > 0) {
                        err = snd_pcm_writei(handle, ptr, cptr);//写音频数据到 PCM 设备
                        if (err == -EAGAIN)
                                continue;
                        if (err < 0) {
...
static void async_callback(snd_async_handler_t *ahandler)
{
        ...
        while (avail >= period_size) {
                ...
                err = snd_pcm_writei(handle, samples, period_size);//写音频数据到 PCM 设备
                if (err < 0) {
...
int main(int argc, char *argv[])
{
        ...
        int err, morehelp;
        snd_pcm_hw_params_t *hwparams;//硬件信息和 PCM 流配置
        ...
        if ((err = snd_pcm_open(&handle, device, SNDRV_PCM_STREAM_PLAYBACK, 0)) < 0) {//打开 PCM， 最后一个参数为 0 意味着标准配置
