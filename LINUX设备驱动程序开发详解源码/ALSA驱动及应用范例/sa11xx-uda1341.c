...
static int snd_sa11xx_uda1341_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	spin_lock(&s->dma_lock);
	switch (cmd) {
	case SNDRV_PCM_TRIGGER_START: //开启 PCM
		/* now we need to make sure a record only stream has a clock */
		if (stream_id == SNDRV_PCM_STREAM_CAPTURE && !s1->active) {//开启录音， 不在播放
		...
		audio_process_dma(s);
		break;
	case SNDRV_PCM_TRIGGER_STOP://停止 PCM
		/* requested stream shutdown */
		audio_stop_dma(s);/* 被请求的流关闭 */
	...
	case SNDRV_PCM_TRIGGER_SUSPEND://挂起
		s->active = 0;
#ifdef HH_VERSION		
		sa1100_dma_stop(s->dmach);//停止 DMA
#else
		//FIXME - DMA API
#endif		
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH://暂停
		...
		break;
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE://暂停释放
		s->active = 1;
		if (s->old_offset) {
			s->tx_spin = 0;
			audio_process_dma(s);
...
static int snd_card_sa11xx_uda1341_open(struct snd_pcm_substream *substream)
{
	...
	if (stream_id == SNDRV_PCM_STREAM_PLAYBACK)//播放子流
...
static struct snd_pcm_ops snd_card_sa11xx_uda1341_playback_ops = {
	...
	.trigger		= snd_sa11xx_uda1341_trigger,//控制播放和录音的启 /停、 挂起/恢复
...
static int __init snd_card_sa11xx_uda1341_pcm(struct sa11xx_uda1341 *sa11xx_uda1341, int device)
{
	...
	if ((err = snd_pcm_new(sa11xx_uda1341->card, "UDA1341 PCM", device, 1, 1, &pcm)) < 0)/* 新建 PCM 设备， playback 和 capture 都为 1 个 */
		return err;
	 ...
	 */
	snd_pcm_lib_preallocate_pages_for_all(pcm, SNDRV_DMA_TYPE_DEV, /* 建立初始缓冲区并设置 dma_type 为 isa */
					      snd_dma_isa_data(),
					      64*1024, 64*1024);

	snd_pcm_set_ops(pcm, SNDRV_PCM_STREAM_PLAYBACK, &snd_card_sa11xx_uda1341_playback_ops); /* 设置 PCM 的操作 */
	...
	/* setup DMA controller *//* 设置 DMA 控制器 */
...
static int __init sa11xx_uda1341_probe(struct platform_device *devptr)
{
	...
	card = snd_card_new(-1, id, THIS_MODULE, sizeof(struct sa11xx_uda1341));/* 新建 card */
	if (card == NULL)
		return -ENOMEM;
	...
	card->private_free = snd_sa11xx_uda1341_free; //card 私有数据释放
	chip->card = card;
	chip->samplerate = AUDIO_RATE_DEFAULT;

	// mixer
	if ((err = snd_chip_uda1341_mixer_new(card, &chip->uda1341)))// 注册 control(mixer) 接口
		goto nodev;

	// PCM
	if ((err = snd_card_sa11xx_uda1341_pcm(chip, 0)) < 0)// 注册 PCM 接口
		goto nodev;
	...
	if ((err = snd_card_register(card)) == 0) {//注册 card
...
{
	snd_card_free(platform_get_drvdata(devptr));//释放 card
