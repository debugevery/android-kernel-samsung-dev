...
{ 
	...
	if (!s->buffers && audio_setup_buf(s)) //设置 DMA 缓冲区
		return -ENOMEM; 
	...
	while (count > 0) { 
		...
		if (audio_channels == 2) { //从用 户 空间复制音频数据
			chunksize = s->fragsize - b->size; 
			if (chunksize > count) 
				chunksize = count; 
			DPRINTK("write %d to %d\n", chunksize, s->buf_idx); 
			if (copy_from_user(b->start + b->size, buffer, chunksize)) { //从用 户 空间复制音频数据
				up(&b->sem); 
				return -EFAULT; 
			} 
			b->size += chunksize; 
		...
		if((ret = s3c2410_dma_enqueue(s->dma_ch, (void *) b, b->dma_addr, b->size))) { //发起 DMA 操作
...
{ 
	const char *buffer0 = buffer; 
	audio_stream_t *s = &input_stream; //得到数据区的指针
	int chunksize, ret = 0; 
	...
	if (!s->buffers) { 
		...
		for (i = 0; i < s->nbfrags; i++) { 
			audio_buf_t *b = s->buf; 
			down(&b->sem); 
			s3c2410_dma_enqueue(s->dma_ch, (void *) b, b->dma_addr, s->fragsize); //依次从缓存区读取数据
			NEXT_BUF(s, buf); 
		} 
	} 
	
	while (count > 0) { //满足用户 的所有读需求
		audio_buf_t *b = s->buf; 
		...
		if (copy_to_user(buffer, b->start + s->fragsize - b->size, chunksize)) { //从缓存区读取数据
			up(&b->sem); 
			return -EFAULT; 
		} 
		...
		buffer += chunksize; 
		count -= chunksize; //已经给用 户 复制了 一部分， count 减少
		if (b->size > 0) { 
			up(&b->sem); 
			break; 
		} 
		...
		s3c2410_dma_enqueue(s->dma_ch, (void *) b, b->dma_addr, s->fragsize); //将缓存区释放
...
{ 
	...
	switch (cmd) { 
	case SOUND_MIXER_INFO: //获得 mixer 信息
		{ 
			mixer_info info; 
	...
	case SOUND_MIXER_READ_STEREODEVS:  //获取设备对立体声的支持
		return put_user(0, (long *) arg); 
		
	case SOUND_MIXER_READ_CAPS: //获取声卡能力
	...
	case SOUND_MIXER_READ_VOLUME: //获取音量
		val = ((63 - uda1341_volume) * 100) / 63; 
		val |= val << 8; 
		return put_user(val, (long *) arg); 
		
	case SOUND_MIXER_READ_IGAIN: //获得增益
		val = ((31- mixer_igain) * 100) / 31; 
		return put_user(val, (int *) arg); 
		
	case SOUND_MIXER_WRITE_IGAIN: //设置增益
		ret = get_user(val, (int *) arg); 
		if (ret) 
			return ret; 
		mixer_igain = 31 - (val * 31 / 100); 
		/* use mixer gain channel 1*/ /* 使用 mixer 增益通道 1 */
		uda1341_l3_address(UDA1341_REG_DATA0); 
...
static struct file_operations smdk2410_audio_fops = { //音频(dsp) 文件操作
...
static struct file_operations smdk2410_mixer_fops = { //混音器文件操作
...
static int s3c2410iis_remove(struct device *dev) { 
	unregister_sound_dsp(audio_dev_dsp); //注销 dsp 和 mixer 设备接口
	unregister_sound_mixer(audio_dev_mixer); 
	audio_clear_dma(&output_stream,&s3c2410iis_dma_out); //注销 DMA 通道
