...
typedef struct {
	...
	TS_RET buf[MAX_TS_BUF];		/* 缓冲区protect against overrun */
	unsigned int head, tail;	/*缓冲区头和尾 head and tail for queued events */
...
#ifdef HOOK_FOR_DRAG 
		ts_timer.expires = jiffies + TS_TIMER_DELAY;//设置定时器定时时间
		add_timer(&ts_timer);
#endif
	} else {
#ifdef HOOK_FOR_DRAG 
...
static int s3c2410_ts_open(struct inode *inode, struct file *filp)
{
	...
	tsdev.penStatus = PEN_UP;
#ifdef HOOK_FOR_DRAG 
	init_timer(&ts_timer);//初始化定时器数据结构的内部值
	ts_timer.function = ts_timer_handler;//设置定时时间到后要执行的函数
#endif
	tsEvent = tsEvent_raw;
	init_waitqueue_head(&(tsdev.wq));//初始化等待队列头
...
static int __init s3c2410_ts_init(void)
{
	...
	tsEvent = tsEvent_dummy;
	//申请设备号
	ret = register_chrdev(0, DEVICE_NAME, &s3c2410_fops);//内核2.4的方式申请设备号，2.6用register_chrdev_region
	if (ret < 0) {
	...
	/* 设置IO口模式set gpio to XP, YM, YP and  YM */
#if 0
	...
	/*置于等待触点中断模式 Wait for touch screen interrupts */
	wait_down_int();
