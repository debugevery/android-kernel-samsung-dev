...
static struct miscdevice s3c2410wdt_miscdev = {//混杂设备
...
static int s3c2410wdt_probe(struct platform_device *pdev)
{
	...
	/* get the memory region内存区域 for the watchdog timer，没有成功则返回 */
	...
	wdt_base = ioremap(res->start, size);//设备内存==》虚拟地址，没有成功则返回
	if (wdt_base == 0) {
	...
	wdt_clock = clk_get(&pdev->dev, "watchdog");	//获得看门狗时钟源
	if (wdt_clock == NULL) {
...
{
	.probe		= s3c2410wdt_probe,//看门狗探测
	.remove		= s3c2410wdt_remove,//看门狗移除
	.shutdown	= s3c2410wdt_shutdown,//看门狗关闭
	.suspend		= s3c2410wdt_suspend,//看门狗挂起
	.resume		= s3c2410wdt_resume,//看门狗恢复
	.driver		= {
				.owner	= THIS_MODULE,
				.name	= "s3c2410-wdt",
				},
